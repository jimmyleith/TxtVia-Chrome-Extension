...
TxtVia.WebDB.createTables = function () {
    TxtVia.WebDB.db.transaction(function (tx) {
        ...
        tx.executeSql('CREATE TABLE IF NOT EXISTS messages (id INTEGER PRIMARY KEY ASC, message_id INTEGER UNIQUE, device_id INTEGER, client_id INTEGER, recipient TEXT, body TEXT, read INTEGER, messaged_at DATEIME, sent_at DATETIME, received_at DATETIME, created_at DATETIME)', [], function (tx, r) {
            console.log('[TxtVia.WebDB.createTables] messages created');
        }, TxtVia.WebDB.onError);
        tx.executeSql('CREATE TABLE IF NOT EXISTS devices (id INTEGER PRIMARY KEY ASC, ' + 'name TEXT, ' + 'device_type TEXT, ' + 'unique_id TEXT UNIQUE, ' + 'carrier TEXT)', [], function (tx, r) {
            console.log('[TxtVia.WebDB.createTables] devices created');
        }, TxtVia.WebDB.onError);
        tx.executeSql('CREATE TABLE IF NOT EXISTS contacts ( ' + 'name TEXT, ' + 'number TEXT UNIQUE)', [], function (tx, r) {
...
TxtVia.WebDB.getMessages = function (recipient, callback) {
    TxtVia.WebDB.db.transaction(function (tx) {
        tx.executeSql('SELECT * FROM `messages` m LEFT JOIN contacts c ON c.number = m.recipient WHERE m.`recipient` = ? ORDER BY `message_at` ASC', [recipient], callback, TxtVia.WebDB.onError);
    });
};
